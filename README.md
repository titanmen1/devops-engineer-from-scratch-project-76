...
**https://hexlet-third-project.crabdance.com/**

Домен (поддомен на FreeDNS/afraid.org) указывает A-записью на внешний IP
балансировщика `51.250.37.125`. NS в облако не делегированы — запись ведётся
в панели регистратора напрямую.

TLS-сертификат Let's Encrypt выпущен через Yandex Certificate Manager
(HTTP-валидация — CNAME/TXT на публичном домене afraid.org запрещены).
Терминирование TLS выполняется на балансировщике (listener :443).
